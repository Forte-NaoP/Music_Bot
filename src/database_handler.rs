...
pub async fn add_title_with_url(conn: &Connection, url: String, title: String) -> Result<(), DBError> {
    conn.call(move |conn| {
        ...
        match _title_id {
            Ok(_) => Err(DBError::OperationFailed(String::from("Title Already Used"))),
            Err(_) => {
                ...
                let url_id = match _url_id {
                    Ok(url_id) => url_id,
                    Err(_) => {
                        tx.execute("INSERT INTO url (url) VALUES (?1)", params![url]).unwrap();
                        tx.query_row("SELECT last_insert_rowid()", params![], |row| row.get::<usize, u64>(0)).unwrap()
                    }
                };
...
pub async fn add_alias_with_title(conn: &Connection, title: String, alias: String) -> Result<(), DBError> {
    conn.call(move |conn| {
        ...
        let _title_id = conn.query_row("SELECT id FROM title WHERE title = (?1)", params![title], |row| row.get::<usize, u64>(0));
        match _alias_id {
            Ok(_) => return Err(DBError::OperationFailed(String::from("Alias Already Used"))),
            Err(_) => {
                let tx = conn.transaction().unwrap();
                let title_id = match _title_id {
                    Ok(title_id) => title_id,
                    Err(_) => return Err(DBError::OperationFailed(String::from("Title Not Found")))
                };
